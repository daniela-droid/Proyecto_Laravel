...
<!-- <div style="background-color: #233858; color: white; padding: 10px 20px; border-radius: 5px;">
    <h1 style="margin: 0; font-size: 1.5rem;">Comarcas</h1>
</div> -->
<h4><strong>Comarcas </strong><i class="fas fa-map text-navy"></i></h4>
@stop

@section('content')

 <!-- Botón Editar -->
           <a href="{{ route('comarcas.create') }}" style="background-color:#233858"class="btn btn-success mb-3">
...
<div class="container">
    @php  
        ...
        if(count($comarcas)>0){
            ...
            foreach($comarcas as $comarca){
                $btnEdit = '<a href="' . route('comarcas.edit', $comarca->id) . '" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Edit">
                                <i class="fa fa-lg fa-fw fa-pen"></i>
                            </a>';
                $btnDelete = '<button class="btn btn-xs btn-default text-danger mx-1 shadow" data-toggle="modal" title="Delete" data-target="#modalDelete-'.$comarca->id.'">
                                <i class="fa fa-lg fa-fw fa-trash"></i>
                            </button>';
                $btnDetails = '<a href="' . route('comarcas.show', $comarca->id) . '" class="btn btn-xs btn-default text-teal mx-1 shadow" title="Details">
        ...
        $config = [
            'data' => $data,
            'order' => [[1, 'asc']],
            'columns' => (count($comarcas) > 0) ? [null, null, null, ['orderable' => false]] : [['orderable' => false]],
        ];
    @endphp
    ...
    <div class="row">
        <div class="col">
            <hr>
            <!-- <x-adminlte-card icon="fas fa-book"  theme="lightblue" title="Listado de Comarcas"> -->
    ...
    @foreach ($comarcas as $comarca)
        <x-delete-modal 
            id="modalDelete-{{ $comarca->id }}"
            :route="route('comarcas.destroy', $comarca->id)"
            :message="'¿Seguro que deseas eliminar la modalidad con el id ' . $comarca->id . '?'"/>
    @endforeach
