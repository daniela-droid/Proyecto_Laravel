...
<!-- <div style="background-color:#233858; color: white; padding: 10px 20px; border-radius: 5px;">
    <h1 style="margin: 0; font-size: 1.5rem; ">Gestión de Reportes </h1>
</div> -->
<h4><strong>Reportes </strong><i class="fas fa-chart-line text-navy"></i></h4>
@stop

@section('content')
<a href="{{ route('reportes.create') }}" class="btn btn-primary mb-3 shadow">
...
<div class="container-fluid">
    @php  
        ...
        foreach($reportes as $reporte) {
            ...
            $categoriaBadge = '<span class="badge badge-'.$badgeColor.'">'.ucfirst($reporte->tipo).'</span>';

            $btnEdit = '<a href="' . route('reportes.edit', $reporte->id) . '" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar"><i class="fa fa-lg fa-fw fa-pen"></i></a>';
           ...
           $btnPdf = '<a href="' . route('reportes.pdf', ['tipo' => 'docente', 'id' => $reporte->id]) . '" class="btn btn-xs btn-default text-danger mx-1 shadow" title="Bajar PDF">
            <i class="fa fa-lg fa-fw fa-file-pdf"></i>
           </a>';
            $data[] = [ 
                $reporte->id,
                $reporte->docentes->Nombre ?? '',
                $reporte->estudiantes->Nombre ?? '',
                "<b>$reporte->titulo</b>",
